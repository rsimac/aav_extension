...
colelem = document.createTextNode("Add the selected flight to the FREQ below and search for next leg from: ")
...
colelem.appendChild(searchbutton)
...
resetbutton.after(undobutton)

hrelem = document.createElement("hr")
undobutton.after(hrelem)
...
chrome.storage.local.get("aavfrep").then((result) => {
    ...
    for (var i=0; i<storedfrep.length; i++) {
        ...
        flightsrow[i].textContent=`${flight.num} ${flight.from}-${flight.to} [${flight.deptz}-${flight.arrtz}] ${flight.dist}`
    }
...
searchbutton.onclick = function() {
    ...
    chrome.storage.local.get("aavfrep").then((result) => {
        ...
        if (storedfrep.length>9) {
            window.alert("Max number of flights (10) reached. Please submit or undo/reset this FREQ.")
        }
        else {
...
for (var i=0; i<flightrows.length; i++) {
    ...
    flight = {
        ...
        "eqpmnt": flightcols[5].innerText.trim(),
        "dist": flightcols[6].innerText.trim()
    }
    ...
    selectbutton.onclick = function() {
        ...
        flightsrow[storedfrep.length].textContent = `${flight.num} ${flight.from}-${flight.to} [${flight.deptz}-${flight.arrtz}] ${flight.dist}`;
